...
from ..models import Classrooms, Teachers, Students, Assignments ,Videos
...
def Video(request):
    if request.method == "POST":
        v=Videos()
        v.caption = request.POST.get('caption')
        if len(request.FILES)!=0:
            v.video=request.FILES['video']
        
        v.save()
        #messages.success(request, 'Video Added Successfully')
        return redirect('/')

    return render(request, 'base/upload_video.html')

@login_required(login_url='login')
def showVideo(request):
    videosss=Videos.objects.all()
    context = {'videosss':videosss}
    return render(request,'base/show_video.html',context)